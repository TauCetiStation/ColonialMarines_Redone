...
/datum/subsystem/job/proc/GiveRandomJob(mob/new_player/player)
	...
	for(var/datum/job/job in shuffle(occupations))
		...
		//if(istype(job, GetJob("Marine"))) // We don't want to give him assistant, that's boring!
		//	continue
...
/datum/subsystem/job/proc/DivideOccupations()
	...
	//	if(PopcapReached())
	//		RejectPlayer(player)
	//	else if(jobban_isbanned(player, "Marine"))
	...
	//	if(PopcapReached())
	//		RejectPlayer(player)
	//	Debug("AC2 Marine located, Player: [player]")
	//	AssignRole(player, "Marine")
	return 1
...
/datum/subsystem/job/proc/EquipRank(mob/living/H, rank, joined_late=0)
	...
	if(job)
		...
		if(config.minimal_access_threshold)
			H << "<FONT color='blue'><B>As this station was initially staffed with a [config.jobs_have_minimal_access ? "full crew, only your jobs necessities" : "skeleton crew, additional access may"] have been added to your ID card.</B></font>"
