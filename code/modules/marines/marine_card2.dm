...
/obj/machinery/computer/marine_card
	...
	//req_one_access = list(access_logistics)
...
/obj/machinery/computer/marine_card/attackby(obj/O, mob/user, params)//TODO:SANITY
...
/obj/machinery/computer/marine_card/proc/job_blacklisted(jobtitle)
	return (jobtitle in blacklisted)


//Logic check for Topic() if you can open the job
/obj/machinery/computer/marine_card/proc/can_open_job(datum/job/job)
	if(job)
...
/obj/machinery/computer/marine_card/proc/can_close_job(datum/job/job)
	if(job)
		if(!job_blacklisted(job.title))
...
/obj/machinery/computer/marine_card/attack_hand(mob/user)
	if(..())
		return
	...
	if (mode == 1) // accessing crew manifest
		...
		var/ID
		if(scan && (access_logistics in scan.access) && !target_dept)
			ID = 1
		else
			ID = 0
		...
		if (authenticated && modify)
			...
			var/accesses = ""
			//accesses += "<h5>Accesses:</h5>"
			//for(var/A in get_all_marine_accesses())
		...
		if ("auth")
			if ((!( authenticated ) && (scan || (istype(usr, /mob/living/silicon))) && (modify || mode)))
				if (check_access(scan))
					...
					if(access_logistics in scan.access)
						if(target_dept)
							head_subordinates = get_marine_jobs()
...
/obj/machinery/computer/marine_card/proc/get_subordinates(rank)
	for(var/datum/job/job in SSjob.occupations)
