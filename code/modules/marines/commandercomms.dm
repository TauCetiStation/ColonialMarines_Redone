...
/*		if("callhiv")
			...
			if(count_observers < 5 || count_humans > count_aliens || SSshuttle.emergency.mode >= SHUTTLE_CALL)
		...
		if("announce")
			if(src.authenticated==2)
				...
				if(!input || !(usr in view(1,src)))
					return
				priority_announce(input, "Commander")//This should really tell who is, IE HoP, CE, HoS, RD, Captain
...
/obj/machinery/computer/commander_communications/proc/call_shuttle_proc(var/mob/user)
	...
	if ((!( ticker ) || SSshuttle.emergency.mode))
		return
	...
	SSshuttle.requestEvac(src, "Request from console.")
	log_game("[key_name(user)] has called the shuttle.")
	message_admins("[key_name_admin(user)] has called the shuttle.", 1)
	//priority_announce("The emergency shuttle has been called. It will arrive in [round(emergency_shuttle.timeleft()/60)] minutes.", "Priority")
